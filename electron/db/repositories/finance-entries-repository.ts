...
export class FinanceEntriesRepository {
  ...
  public listByTruckId(truckId: string): FinanceEntryRecord[] {
    const rows = this.database
      .prepare(
        'SELECT * FROM finance_entries WHERE truck_id = ? ORDER BY occurred_at DESC',
      )
  ...
  public listByTripId(tripId: string): FinanceEntryRecord[] {
    const rows = this.database
      .prepare(
        'SELECT * FROM finance_entries WHERE trip_id = ? ORDER BY occurred_at DESC',
      )
